...
class InteractiveDebuggerPlugin(base_plugin.TBPlugin):
    ...
    def listen(self, grpc_port):
        ...
        sys.stderr.write(
            "Creating InteractiveDebuggerPlugin at port %d\n" % self._grpc_port
        )
        sys.stderr.flush()
        self._debugger_data_server = interactive_debugger_server_lib.InteractiveDebuggerDataServer(
            self._grpc_port
        )
    ...
    def _serve_gated_grpc(self, request):
        ...
        if mode == "retrieve_all" or mode == "retrieve_device_names":
            ...
            for device_name in debug_graph_defs:
                gated[
                    device_name
                ] = self._debugger_data_server.get_gated_grpc_tensors(
                    run_key, device_name
                )
    ...
    def _serve_debugger_grpc_host_port(self, request):
        return http_util.Respond(
            request,
            {"host": platform.node(), "port": self._grpc_port},
            "application/json",
        )
    ...
    def _serve_tensor_data(self, request):
        ...
        try:
            ...
            response = {"tensor_data": sliced_tensor_data, "error": None}
            status_code = 200
            ...
            status_code = 500
        return http_util.Respond(
            request, response, response_encoding, status_code
        )
    ...
    def _serve_source_code(self, request):
        ...
        if mode == "paths":
            ...
            response = {
                "paths": self._debugger_data_server.query_source_file_paths()
            }
            ...
            response = {
                ...
                "lineno_to_op_name_and_stack_pos": self._debugger_data_server.query_file_tracebacks(
                    file_path
                ),
            }
            return http_util.Respond(request, response, response_encoding)
            ...
            response = {
                "op_traceback": {
                    op_name: self._debugger_data_server.query_op_traceback(
                        op_name
                    )
                }
            }
            ...
            response = {
                "error": "Invalid mode for source_code endpoint: %s" % mode
            }
