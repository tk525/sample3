...
def parse_time_indices(s):
    ...
    if len(parsed) != 1:
        raise ValueError(
            "Invalid number of slicing objects in time indices (%d)"
            % len(parsed)
        )
    else:
        return parsed[0]
...
def array_view(array, slicing=None, mapping=None):
    ...
    sliced_array = (
        array[command_parser._parse_slices(slicing)] if slicing else array
    )
    ...
    if np.isscalar(sliced_array) and str(dtype) == "string":
        ...
        if mapping == "image/png":
            if len(sliced_array.shape) == 2:
                ...
                raise ValueError(
                    "Invalid rank for image/png mapping: %d"
...
<<<<<<< HEAD
    ...
    is_negative_infinity = np.logical_and(
        is_infinity, np.logical_not(is_positive)
    )
    ...
    if np.size(finite_indices):
        ...
        scaled = np.array(
            (array - minval) / (maxval - minval) * 255, dtype=np.uint8
        )
        rgb = np.repeat(
            np.expand_dims(scaled, -1), IMAGE_COLOR_CHANNELS, axis=-1
        )
    else:
