...
<<<<<<< HEAD
    ...
    def __init__(
        ...
        if events_file_names:
            self._events_file_count = (
                self._obtain_file_index(events_file_names[-1]) + 1
            )
        ...
        try:
            ...
            if self._event_count == self._check_this_often:
                ...
                file_path = os.path.join(
                    self._events_directory, self.get_current_file_name()
                )
                if not tf.io.gfile.exists(file_path):
                    ...
                    > self._single_file_size_cap_bytes
                ):
    ...
    def dispose(self):
        ...
        Call this method when this object is done being used in order to
        clean up resources and handlers. This method should ever only be
        called once.
        """
        self._lock.acquire()
        self._events_writer.Close()
    ...
    def _fetch_events_files_on_disk(self):
        """Obtains the names of debugger-related events files within the
        directory.
