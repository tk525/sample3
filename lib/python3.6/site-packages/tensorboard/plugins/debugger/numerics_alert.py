...
HistoryTriplet = collections.namedtuple(
    "HistoryTriplet", ["device", "tensor", "jsonable_history"]
)
...
class NumericsAlertHistory(object):
    ...
    def first_timestamp(self, event_key=None):
        ...
        if event_key is None:
            timestamps = [
                self._trackers[key].first_timestamp for key in self._trackers
            ]
    ...
    def create_jsonable_history(self):
        ...
        """
        return {
            value_category_key: tracker.get_description()
...
class NumericsAlertRegistry(object):
    ...
    def __init__(self, capacity=100, initialization_list=None):
        ...
        if initialization_list:
            ...
            for entry in initialization_list:
                ...
                self._data[
                    (triplet.device, triplet.tensor)
                ] = NumericsAlertHistory(
        ...
        for key in self._data:
            ...
            report.append(
                NumericsAlertReportRow(
                    ...
                    neg_inf_event_count=history.event_count(
                        constants.NEG_INF_KEY
                    ),
                    pos_inf_event_count=history.event_count(
                        constants.POS_INF_KEY
                    ),
                )
            )
    ...
    def create_jsonable_registry(self):
        ...
        return [
            HistoryTriplet(pair[0], pair[1], history.create_jsonable_history())
