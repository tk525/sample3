...
class DebuggerDataServer(grpc_debug_server.EventListenerBaseServicer):
    ...
    def __init__(self, receive_port, logdir, always_flush=False):
        ...
        if not tf.io.gfile.exists(debugger_directory):
            try:
                ...
                logger.info(
                    "Created directory for debugger data: %s",
                    debugger_directory,
                )
        ...
        self._events_writer_manager = events_writer_manager_lib.EventsWriterManager(
            events_directory=debugger_directory, always_flush=always_flush
        )
        ...
        try:
            ...
            logger.error(
                ...
                self._events_writer_manager.get_current_file_name(),
                e,
            )
        ...
        if tf.io.gfile.exists(self._registry_backup_file_path):
            ...
            with tf.io.gfile.GFile(
                self._registry_backup_file_path, "r"
            ) as backup_file:
                try:
                    ...
                    logger.error(
                        ...
                        self._registry_backup_file_path,
                        err,
                    )
    ...
    def start_the_debugger_data_receiving_server(self):
        ...
        After this method is called, health pills issued to
        host:receive_port will be stored by this object. Calling this
        method also creates a file within the log directory for storing
        health pill summary events.
        """
        self.run_server()

    def get_events_file_name(self):
        """Gets the name of the debugger events file currently being written
        to.
