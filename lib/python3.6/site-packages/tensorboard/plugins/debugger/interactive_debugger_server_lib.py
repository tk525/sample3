...
<<<<<<< HEAD
This listener server writes debugging-related events into a logdir
directory, from which a TensorBoard instance can read.
=======
...
class RunStates(object):
    ...
    def add_graph(self, run_key, device_name, graph_def, debug=False):
        ...
        """
        graph_dict = (
            self._run_key_to_debug_graphs
            if debug
            else self._run_key_to_original_graphs
        )
        if not run_key in graph_dict:
            graph_dict[run_key] = dict()  # Mapping device_name to GraphDef.
        graph_dict[run_key][
            tf.compat.as_str(device_name)
    ...
    def get_graph(self, run_key, device_name, debug=False):
        """Get the runtime GraphDef proto associated with a run key and a
        device.
    ...
    def get_gated_grpc_tensors(self, run_key, device_name):
        return self._run_key_to_debug_graphs[run_key][
            device_name
        ].get_gated_grpc_tensors()

    def get_maybe_base_expanded_node_name(
        self, node_name, run_key, device_name
    ):
        ...
        return self._run_key_to_original_graphs[run_key][
            device_name
        ].maybe_base_expanded_node_name(node_name)
...
class InteractiveDebuggerDataStreamHandler(
    ...
    def _is_debug_node_in_breakpoints(self, debug_node_key):
        ...
        return (
            ...
            output_slot,
            debug_op,
        ) in self._run_states.get_breakpoints()
=======
...
<<<<<<< HEAD
    """Manages source files and tracebacks involved in the debugged TF
    program."""
    ...
    def add_debugged_source_file(self, debugged_source_file):
        ...
        self._source_file_last_modified[
            key
        ] = debugged_source_file.last_modified
    ...
    def get_file_tracebacks(self, file_path):
        ...
        for op_log_entry in self._graph_traceback.log_entries:
            for stack_pos, trace in enumerate(op_log_entry.code_def.traces):
                if (
                    self._graph_traceback.id_to_string[trace.file_id]
                    == file_path
                ):
                    if (
                        trace.lineno
                        not in lineno_to_op_names_and_stack_position
                    ):
...
class InteractiveDebuggerDataServer(
    ...
    """A service that receives and writes debugger data such as health
    pills."""
    ...
    def SendTracebacks(self, request, context):
        self._source_manager.add_graph_traceback(
            request.graph_version, request.graph_traceback
        )
        return debug_service_pb2.EventReply()
    ...
    def query_tensor_store(
        self, watch_key, time_indices=None, slicing=None, mapping=None
    ):
        ...
        Returns:
          ...
          If mapping is not `None`, the format of the return value will depend on
            the mapping.
        """
        return self._tensor_store.query(
            watch_key,
    ...
    def dispose(self):
        ...
        Call only after this is done being used.
        """
