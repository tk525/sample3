...
HealthPillEvent = collections.namedtuple(
    "HealthPillEvent",
    [
        "wall_time",
        "step",
...
class DebuggerPlugin(base_plugin.TBPlugin):
    ...
    def listen(self, grpc_port):
        ...
        threading.Thread(
            target=self._debugger_data_server.start_the_debugger_data_receiving_server
        ).start()
    ...
    def is_active(self):
        ...
        """
        return bool(
            self._grpc_port is not None
            and self._event_multiplexer
            and self._event_multiplexer.PluginRunToTagToContent(
    ...
    def _serve_health_pills_handler(self, request):
        ...
        try:
            ...
            response = ("Could not decode node name JSON string %r: %s") % (
        ...
        if not isinstance(node_names, list):
            response = ("%r is not a JSON list of node names:") % (
                jsonified_node_names
            )
            return http_util.Respond(request, response, "text/plain", code=400)
        ...
        if step_string is None:
            ...
            try:
                ...
                response = "Error retrieving health pills for step %d: %s" % (
                    step,
                    error,
                )
                return http_util.Respond(
                    request, response, "text/plain", code=404
                )
        ...
        for node_name in node_names:
            ...
            for tensor_event in self._event_multiplexer.Tensors(run, node_name):
                ...
                try:
                    ...
                    health_pills.append(
                        self._tensor_proto_to_health_pill(
    ...
    def _tensor_proto_to_health_pill(
        self, tensor_event, node_name, device, output_slot
    ):
    ...
    def _process_health_pill_event(
        self, node_name_set, mapping, target_step, file_path
    ):
    ...
    def _process_health_pill_value(
        self,
        wall_time,
        step,
        device_name,
        ...
        """Creates a HealthPillEvent containing various properties of a health
        pill.
